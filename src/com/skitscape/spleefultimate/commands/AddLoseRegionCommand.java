...
public class AddLoseRegionCommand extends SubCommand
{
  public boolean onCommand(Player player, String[] args)
  {
    ...
    {
      player.sendMessage(MessageFormatter.format(Messages.getMessage("error-loseregion_alreadyexists"), "{ID}", args[1]));

      return true;
    }

    game.getArena().addLoseRegion(args[1], selection);

    player.sendMessage(MessageFormatter.format(Messages.getMessage("message-loseregion_added"), "{ID}", args[1]));
