...
public class AddSpleefRegionCommand extends SubCommand
{
  public boolean onCommand(Player player, String[] args)
  {
    ...
    if (!player.hasPermission("spleefultimate.admin.addspleefregion"))
    {
    ...
    {
      player.sendMessage(MessageFormatter.format(Messages.getMessage("error-spleefregion_alreadyexists"), "{ID}", args[1]));

      return true;
    }

    game.getArena().addSpleefRegion(args[1], selection);

    player.sendMessage(MessageFormatter.format(Messages.getMessage("message-spleefregion_added"), "{ID}", args[1]));
  ...
  {
    return "/spleef addspleefregion <gameId> <regionId>";
  }
}